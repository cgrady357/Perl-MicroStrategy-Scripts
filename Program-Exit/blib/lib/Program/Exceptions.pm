package Program::Exit::Exceptions;
...
BEGIN
{
    %e = ( 'Program::Exit::Exception' =>
           { description => 'generic base class for all Program::Exit exceptions',
             alias => 'error'},

           'Program::Exit::Exception::Abort' =>
           { isa => 'Program::Exit::Exception',
             fields => [qw(aborted_value)],
             description => 'a component called $m->abort' },

           'Program::Exit::Exception::Decline' =>
           { isa => 'Program::Exit::Exception',
             fields => [qw(declined_value)],
             description => 'a component called $m->decline' },

           'Program::Exit::Exception::Compiler' =>
           { isa => 'Program::Exit::Exception',
             alias => 'compiler_error',
             description => 'error thrown from the compiler' },

           'Program::Exit::Exception::Compilation' =>
           { isa => 'Program::Exit::Exception',
             alias => 'compilation_error',
             fields => [qw(file_name)],
             description => "error thrown in eval of the code for a component" },

           'Program::Exit::Exception::Compilation::IncompatibleCompiler' =>
           { isa => 'Program::Exit::Exception::Compilation',
           ...
           'Program::Exit::Exception::Params' =>
           { isa => 'Program::Exit::Exception',
             alias => 'param_error',
             description => 'invalid parameters were given to a method/function' },
	   
           'Program::Exit::Exception::Email' =>
           { isa => 'Program::Exit::Exception',
             alias => 'email_error',
             description => 'error thrown from email program' },

           'Program::Exit::Exception::IO' =>
           { isa => 'Program::Exit::Exception',
             alias => 'io_error',
             description => 'indicates that some input/output-related trouble has occurred' },

           'Program::Exit::Exception::IO::File' =>
           { isa => 'Program::Exit::Exception::IO',
             alias => 'file_error',
	     fields => [qw(file_name)],
             description => 'Indicates that a file could not be opened' },

          'Program::Exit::Exception::IO::File::Copy' =>
           { isa => 'Program::Exit::Exception::IO::File',
             alias => 'file_copy_error',
             description => 'Indicates that a file could not be copied' },

           'Program::Exit::Exception::CommandManager' =>
           { isa => 'Program::Exit::Exception',
             alias => 'cmdmgr_error',
             description => 'indicates that some Command Manager related trouble has occurred' },

           'Program::Exit::Exception::Syntax' =>
           { isa => 'Program::Exit::Exception',
             alias => 'syntax_error',
             fields => [qw(source_line comp_name line_number)],
             description => 'invalid syntax was found in a component' },

           'Program::Exit::Exception::System' =>
           { isa => 'Program::Exit::Exception',
             alias => 'system_error',
             description => 'a system call of some sort failed' },

           'Program::Exit::Exception::TopLevelNotFound' =>
           { isa => 'Program::Exit::Exception',
             alias => 'top_level_not_found_error',
             description => 'the top level component could not be found' },

           'Program::Exit::Exception::VirtualMethod' =>
           { isa => 'Program::Exit::Exception',
             alias => 'virtual_error',
...
Program::Exit::Exception->Trace(1);

# To avoid circular reference between exception and request.
Program::Exit::Exception->NoRefs(1);

# The import() method allows this:
#  use Program::Exit::Exceptions(abbr => ['error1', 'error2', ...]);
# ...
...
sub import
{
    ...
    {
        no strict 'refs';
        *{"${caller}::isa_program_exit_exception"} = \&isa_program_exit_exception;
        *{"${caller}::rethrow_exception"} = \&rethrow_exception;
    }
}

sub isa_program_exit_exception
{
    my ($err, $name) = @_;
    return unless defined $err;

    $name = $name ? "Program::Exit::Exception::$name" : "Program::Exit::Exception";
    no strict 'refs';
    die "no such exception class $name" unless $name->isa('Program::Exit::Exception');
...
sub rethrow_exception
{
    ...
    Program::Exit::Exception->throw(error => $err);
}

package Program::Exit::Exception;

use Program::Exit::MethodMaker
...
# If we create a new exception from a Program::Exit exception, just use the
...
sub throw
{
    ...
    if (Program::Exit::Exceptions::isa_program_exit_exception($params{error})) {
        $params{error} = $params{error}->error;
    }
    if (Program::Exit::Exceptions::isa_program_exit_exception($params{message})) {
...
sub filtered_frames
{
    ...
    my %ignore_subs = map { $_ => 1 }
        qw[
           ...
           Program::Exit::Commands::__ANON__
           Program::Exit::Component::run
           Program::Exit::Exception::throw
           Program::Exit::Exceptions::__ANON__
           Program::Exit::Request::_run_comp
           ];
    while (my $frame = $trace->next_frame)
    {
        last if ($frame->subroutine eq 'Program::Exit::Request::exec');
        unless ($frame->filename =~ /Exit\/Exceptions\.pm/ or
                $ignore_subs{ $frame->subroutine } or
                ($frame->subroutine eq 'Program::Exit::Request::comp' and $frame->filename =~ /Request\.pm/)) {
            push(@frames, $frame);
        }
    }
    @frames = grep { $_->filename !~ /Exit\/Exceptions\.pm/ } $trace->frames if !@frames;
    return @frames;
}
...
sub analyze_error
{
    ...
    if ($self->isa('Program::Exit::Exception::Syntax')) {
        $file = $self->comp_name;
        push(@lines, $self->line_number);
    } elsif ($self->isa('Program::Exit::Exception::Compilation')) {
        $file = $self->filename;
...
sub as_html
{
    ...
    my $out;
    my $interp = Program::Exit::Interp->new(out_method => \$out);
...
% Program::Exit::Escapes::basic_html_escape(\$msg);
% $msg =~ s/\n/<br>/g;
...
%       Program::Exit::Escapes::basic_html_escape(\$line);
    <tr>
...
%        my $f = $frame->filename; Program::Exit::Escapes::basic_html_escape(\$f);
%        my $l = $frame->line; Program::Exit::Escapes::basic_html_escape(\$l);
        <% $f %>:<% $l %><br>
%    }
  </td>
 </tr>
...
% Program::Exit::Escapes::basic_html_escape(\$raw);
% $raw =~ s/\t//g;
...
package Program::Exit::Exception::Compilation;
...
package Program::Exit::Exception::Syntax;
...
Program::Exit::Exceptions - Exception objects thrown by Mason

=head1 SYNOPSIS

  use Program::Exit::Exceptions ( abbr => [ qw(system_error) ] );

  open FH, 'foo' or system_error "cannot open foo: $!";

=head1 DESCRIPTION

This module creates the hierarchy of exception objects used by Program::Exit,
...
function to throw a C<Program::Exit::Exception::System> exception.
...
=item Program::Exit::Exception::Compiler
...
=item Program::Exit::Exception::Compilation::IncompatibleCompiler
...
an C<Program::Exit::Exception::Syntax> exception, you call the
...
for the L<Request object|Program::Exit::Request> such as C<text> or
C<html>.

You can create your own method in the C<Program::Exit::Exception>
...
This module also exports the C<isa_program_exit_exception> function.  This
...
So it can be called either as:
  ...
  if ( isa_program_exit_exception($@) ) { ... }

or

  if ( isa_program_exit_exception($@, 'Syntax') ) { ... }

Note that when specifying a subclass you should not include the
leading "Program::Exit::Exception::" portion of the class name.
