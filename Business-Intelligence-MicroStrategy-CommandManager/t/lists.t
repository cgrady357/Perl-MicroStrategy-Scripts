...
is(
    $foo->list_acl_properties( 	OBJECT_TYPE => "FOLDER",
...
is($foo->list_all_dbinstances, 'LIST ALL DBINSTANCES;', "list_all_dbinstances");

is( $foo->list_all_dblogins, 'LIST ALL DBLOGINS;', "list_all_dblogins");

is($foo->list_all_servers, 'LIST ALL SERVERS;', "list_all_servers");

is($foo->list_attribute_properties(	ATTRIBUTE => "attribute_name", 
					LOCATION => "location_path", 
					PROJECT => "project_name"
				), 
...
is($foo->list_attributes(	LOCATION => "location_path", 
			PROJECT => "project_name" ), 
'LIST ALL ATTRIBUTES IN FOLDER "location_path" FOR PROJECT "project_name";',
"list_attributes");

is(
$foo->list_caching_properties(	CACHE_TYPE => "REPORT",
				PROJECT => "project_name"
			     ),
'LIST ALL PROPERTIES FOR REPORT CACHING IN PROJECT "project_name";',
"list_caching_properties");

is($foo->list_custom_group_properties(	CUSTOMGROUP => "customgroup_name", 
					LOCATION => "location_path", 
					PROJECT => "project_name"
				  ),
...
is($foo->list_custom_groups(	LOCATION => "location_path", 
				PROJECT => "project_name"
			),
'LIST ALL CUSTOMGROUPS IN FOLDER "location_path" FOR PROJECT "project_name";',
"list_custom_groups");

is($foo->list_dbconnection_properties(	DBCONNECTION => "dbConnection_name"),
'LIST ALL PROPERTIES FOR DBCONNECTION "dbConnection_name";',
"list_dbconnection_properties1");

is($foo->list_dbconnection_properties(	PROPERTIES => [ qw(ID NAME ODBCDSN DEFAULTLOGIN DRIVERMODE EXECMODE MAXCANCELATTEMPT MAXQUERYEXEC CHARSETENCODING TIMEOUT IDLETIMEOUT) ],
...
is($foo->list_facts(	LOCATION => "location_path", 
			PROJECT => "project_name"
		),
...
is($foo->list_filters(	LOCATION => "location_path", 
			OWNER => "login_name", 
			PROJECT => "project_name"
		  ),
...
is($foo->list_folders(	LOCATION => "location_path", 
			PROJECT => "project_name"
		  ),
'LIST ALL FOLDERS IN "location_path" FOR PROJECT "project_name";',
"list_folders");

is($foo->list_job_properties("job_id"), 'LIST ALL PROPERTIES FOR JOB job_id;', "list_job_properties");

is($foo->list_jobs( TYPE => "ALL", 
		 USER => "login_name"), 
'LIST ALL JOBS FOR USER "login_name";', 
"list_jobs");

is($foo->list_lock_properties("project_name"), 
...
is($foo->list_project_config_properties("project_name"),
'LIST ALL PROPERTIES FOR PROJECT CONFIGURATION FROM PROJECT "project_name";',
"list_project_config_properties");

is(
$foo->list_project_properties(	PROJECT => "project_name"
			     ),
'LIST ALL PROPERTIES FOR PROJECT "project_name";',
"list_project_properties1");

is(
$foo->list_project_properties(	PROPERTIES => [ qw(DESCRIPTION NAME ID CREATIONTIME MODIFICATIONTIME) ], 
				PROJECT => "project_name"
			     ),
'LIST DESCRIPTION, NAME, ID, CREATIONTIME, MODIFICATIONTIME FOR PROJECT "project_name";',
"list_project_properties2");

is($foo->list_projects_cluster, 'LIST ALL PROJECTS FOR CLUSTER;', "list_projects_cluster");

is($foo->list_projects("REGISTERED"),'LIST REGISTERED PROJECTS;', "list_projects");

is($foo->list_report_cache_properties(	REPORT_CACHE => "cache_name", 
					PROJECT => "project_name"
				  ),
'LIST ALL PROPERTIES FOR REPORT CACHE "cache_name" IN PROJECT "project_name";',
"list_report_cache_properties");

is($foo->list_report_caches("project_name"),
...
is($foo->list_schedule_properties("schedule_name"), 
...
is(
$foo->list_security_filter_properties( 	SECURITY_FILTER => "sec_filter_name", 
					LOCATION => "location_path", 
					PROJECT => "project_name"
				     ),
...
is(
$foo->list_security_filters(	USER_OR_GROUP => "USER",
...
is(
$foo->list_security_role_properties(	PROPERTIES => [ qw(NAME ID DESCRIPTION)], 
...
is(
$foo->list_security_role_properties(	SECURITY_ROLE => "sec_role_name"
				     ),
'LIST ALL PROPERTIES FOR SECURITY ROLE "sec_role_name";',
"list_security_role_properties2");

is($foo->list_security_role_privileges("sec_role_name"), 'LIST ALL PRIVILEGES FOR SECURITY ROLE "sec_role_name";', "list_security_role_privileges");

is($foo->list_security_roles, 'LIST ALL SECURITY ROLES;', "list_security_roles");

is($foo->list_server_config_properties("ALL"), 
'LIST ALL PROPERTIES FOR SERVER CONFIGURATION;',
"list_server_config_properties1");

is($foo->list_server_config_properties([qw(MAXCONNECTIONTHREADS BACKUPFREQ USENTPERFORMANCEMON USEMSTRSCHEDULER BALSERVERTHREADS)]), 'LIST MAXCONNECTIONTHREADS, BACKUPFREQ, USENTPERFORMANCEMON, USEMSTRSCHEDULER, BALSERVERTHREADS FOR SERVER CONFIGURATION;',
"list_server_config_properties2");

is($foo->list_server_properties("machine_name"),'LIST ALL PROPERTIES FOR SERVER "machine_name";', "list_server_properties");

is($foo->list_servers_cluster, 'LIST ALL SERVERS IN CLUSTER;', "list_servers_cluster");
...
is($foo->list_statistics_properties("project_name"),
...
is($foo->list_tables(	LOCATION => "location_path", 
			PROJECT => "project_name"
		 ),
...
is($foo->list_user_conn_properties( USER => "login_name"),
'LIST ALL PROPERTIES FOR USER CONNECTION "login_name";',
"list_user_conn_properties1");

is($foo->list_user_conn_properties( SESSIONID => "sessionID" ),
'LIST ALL PROPERTIES FOR USER CONNECTION SESSIONID sessionID;',
"list_user_conn_properties1");

is($foo->list_user_connections("ALL"), 'LIST ALL USER CONNECTIONS;', "list_user_connections");

is($foo->list_user_group_members("user_group_name"),'LIST MEMBERS FOR USER GROUP "user_group_name";',"list_user_group_members");

is($foo->list_user_group_privileges("user_group_name"), 'LIST ALL PRIVILEGES FOR USER GROUP "user_group_name";',
	"list_user_group_privileges");

is(
$foo->list_user_group_properties(	USER_GROUP => "user_group_name"  ),
'LIST ALL PROPERTIES FOR USER GROUP "user_group_name";', 
"list_user_group_properties1");

is(
$foo->list_user_group_properties(	PROPERTIES => [qw(DESCRIPTION NAME ID LDAPLINK MEMBERS)], 
...
is($foo->list_user_privileges(	TYPE => "ALL", 
				USER => "login_name"),
'LIST ALL PRIVILEGES FOR USER "login_name";',
"list_user_privileges");

is($foo->list_user_profiles( 	USER => "ALL" ), 'LIST ALL PROFILES FOR USERS;', "list_user_profiles1");
...
is(
$foo->list_user_profiles( 	USER => "Developer" ), 'LIST ALL PROFILES FOR USER "Developer";',
"list_user_profiles4");
...
is($foo->list_user_properties(	PROPERTIES => [qw(FULLNAME ENABLED NTLINK LDAPLINK WHLINK DESCRIPTION ALLOWCHANGEPWD ALLOWSTDAUTH PASSWORDEXP PASSWORDEXPFREQ NAME ID GROUPS CHANGEPWD) ], 
				USER => "login_name" ),
...
is($foo->list_user_properties(	USER_GROUP => "user_group_name" ),
'LIST ALL PROPERTIES FOR USERS IN GROUP "user_group_name";',
"list_user_properties2");

is($foo->list_whtables("project_name"), 'LIST ALL AVAILABLE WHTABLES FOR PROJECT "project_name";',"list_whtables");
