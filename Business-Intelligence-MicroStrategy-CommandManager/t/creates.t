...
is(
 $foo->create_attribute(ATTRIBUTE => "Day", 
...
is(
   $foo->create_custom_group(	CUSTOMGROUP => "customgroup_name", 
				...
				OUTPUTLEVEL_LOCATIONS => [ "outputlevel_location_path1", "outputlevel_location_pathN"], 
...
is(
  $foo->create_schedule(SCHEDULE => "Schedule9",
...
is(
$foo->create_schedule_relation(	SCHEDULE => "Schedule1", 
				USER_OR_GROUP => "USER", 
...
is(
$foo->create_shortcut(	LOCATION => "location_path", 
...
is(
    $foo->create_user(	USER => "login_name",
			NTLINK => "nt_user_id", 
...
is(
    $foo->create_user(	USER => '\DomainName\UserName',
	    		IMPORTWINUSER => "TRUE",
		),
