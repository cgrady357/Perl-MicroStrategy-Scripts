...
is($foo->load_project("project_name"), 'LOAD PROJECT "project_name";',"load_project");

is($foo->load_projects_cluster(	PROJECT => "project_name", 
				SERVERS =>  "ALL" ),
...
is($foo->log_event(	MESSAGE => "event_message", 
			TYPE => "ERROR" ),
'LOG EVENT "event_message" TYPE ERROR;',
"log_event");

is($foo->purge_caching(	TYPE => "REPORT",
			PROJECT => "project_name" ),
'PURGE REPORT CACHING IN PROJECT "project_name";',
"purge_caching");

is($foo->purge_statistics( 	START_DATE => "start_date", 
				END_DATE => "end_date", 
				TIMEOUT => "seconds"
		      ),
...
is($foo->restart_server("machine_name"), 'RESTART SERVER IN "machine_name";', "restart_server");

is($foo->resume_project("project_name"), 'RESUME PROJECT "project_name";',"resume_project");

is(
$foo->revoke_privileges(PRIVILEGE => "ALL", 
			USER => "login_name" ),
'REVOKE ALL PRIVILEGES FROM USER "login_name";',
"revoke_privileges1");

is(
$foo->revoke_privileges(PRIVILEGE => ["privilege1", "privilegeN"], 
...
is($foo->revoke_security_roles(	SECURITY_ROLE => "sec_role_name",
				USER_OR_GROUP => "USER", 
...
is($foo->run_command("executable_program"), 'RUN COMMAND "executable_program";', "run_command");

is($foo->send_message( 	MESSAGE => "message", 
			USER => "ALL" ),
'SEND MESSAGE "message" TO ALL USERS;',
"send_message");

is($foo->set_property_hidden(	HIDDEN => "TRUE", 
...
is($foo->start_server("machine_name"), 'START SERVER IN "machine_name";', "start_server");

is($foo->start_service(	SERVICE => "service_name", 
			SERVER => "machine_name"  ),
'START SERVICE "service_name" IN "machine_name";',
"start_service");

is( $foo->stop_server("machine_name"),'STOP SERVER IN "machine_name";', "stop_server");

is($foo->stop_service(	SERVICE => "service_name", 
			SERVER => "machine_name"
		  ),
...
is(
$foo->take_ownership(	OBJECT_TYPE => "conf_object_type", 
			OBJECT_NAME => "object_name",
			LOCATION => "location_path",
			PROJECT => "project_name"
		    ),
...
is(
$foo->take_ownership(	OBJECT_TYPE => "FOLDER",
...
is($foo->trigger_event("event_name"),'TRIGGER EVENT "event_name";', "trigger_event");

is($foo->unload_project("project_name"), 'UNLOAD PROJECT "project_name";', "unload_project");
...
is($foo->unlock_configuration, 'UNLOCK CONFIGURATION FORCE;', "unlock_configuration");

is($foo->unlock_project("project_name"), 'UNLOCK PROJECT "project_name" FORCE;', "unlock_project");

is($foo->unregister_project("project_name"), 'UNREGISTER PROJECT "project_name";', "unregister_project");

is($foo->update_project("project_name"), 'UPDATE PROJECT "project_name";', "update_project");

is($foo->update_schema(	REFRESHSCHEMA => "TRUE",
			RECALTABLEKEYS	 => "TRUE",
...
is($foo->update_schema(	REFRESHSCHEMA => "FALSE",
...
is(
$foo->update_structure(	COLMERGEOPTION => "RECENT",
...
is(
$foo->update_structure(	COLMERGEOPTION => "MAXDENOMINATOR",
			PROJECT => "project_name"
		      ),
