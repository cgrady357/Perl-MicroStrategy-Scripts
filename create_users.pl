...
# perl create_users.pl --cfg_file "C:\\users.ini" --job jobname
my $usage
    = "perl create_users.pl --cfg_file configuration_file --job control_m_job_name\n";
...
my $user_info_file   = $cfg->val( $job, "user_info_file" );
my $spreadsheet_page = $cfg->val( $job, "spreadsheet_page" );
my $user_group       = $cfg->val( $job, "user_group" );
...
$log->info( "Set input file " . $cmdmgr->{'scp'} );
...
$log->info( "Set results file " . $cmdmgr->{'out'});
$log->info( "Set fail file " . $cmdmgr->{'nok'} );
$log->info( "Set success file " . $cmdmgr->{'ok'});
eval {
...
$log->info("Creating script " . $cmdmgr->{scp});
eval {
    my $usr_list = xls2aref( { user_info_file   => $user_info_file,
                               spreadsheet_page => $spreadsheet_page,
                             }
    );
    for my $user (@$usr_list) {
        ( my $name = $user->[1] ) =~ s/["]//g;
        print $fh $msi->create_user( USER           => $user->[0],
                                     FULLNAME       => $name,
                                     PASSWORD       => $user->[2],
                                     WHLINK         => $user->[0],
                                     WHPASSWORD     => $user->[2],
...
if ($@) {
    abnormal_exit( { ERROR         => "create_user error",
                     ERROR_MESSAGE => $@,
                   }
    );
}

$log->info( "Set project source name , command manager id, and password");
eval {
    $msi->set_connect(
...
sub xls2aref {
    my $self = shift;
    if ( !-e $self->{user_info_file} ) {
        abnormal_exit( { ERROR         => "file existence test error",
                         ERROR_MESSAGE => "Can't find user_info_file: "
                             . $self->{user_info_file},
                       }
        );
    }
    my $xls = Spreadsheet::ParseExcel::Simple->read( $self->{user_info_file} )
        or abnormal_exit( {
              ERROR         => "Spreadsheet::ParseExcel::Simple read error",
              ERROR_MESSAGE => "Can't read file: " . $self->{user_info_file},
            }
        );
    my $cnt  = 0;
    my $data = [];
    for my $sheet ( $xls->sheets ) {
        next unless $cnt++ == $self->{spreadsheet_page};
