...
MicroStrategy::Cmdmgr - The MicroStrategy Command Manager module
...
use MicroStrategy::Cmdmgr;

my $foo = MicroStrategy::Cmdmgr->new();

my $script = "del_usr.scp";

my $fh;

open($fh, ">", $script) or die($!, $?);

my $user_list = ["jen", "kris", "sara", "tabitha"];

for my $user(@$user_list){ print $fh $foo->delete_user($user); }

$foo->set_connect("project_source_name", "userid", "password");

$foo->set_inputfile($script);

$foo->set_resultsfile("results.log", "fail.log", "success.log");
...
Use this module to create or execute MicroStrategy Command Manager scripts.
...
Username = Identifies the user name to be associated with the project source name.
...
SuccessFile = Success log file name. You may only use successfile with the results file switch.

$foo->set_resultsfile("results.out","fail.out","success.out");

=cut

sub set_resultsfile {
	my $self = shift;
	$self->{ResultsFile} = "-or " . shift;
	$self->{FailFile} = "-of " . shift;
       	$self->{SuccessFile} = "-os " . shift;
}
...
Displays the contents of a MicroStrategy::Cmdmgr object. 
...
sub run_script {
	...
	for my $req(qw(cmdmgr_exe ProjectSourceName Username Password)) {
		if($self->{$req}) { $args .= " " . $self->{$req}; } else { croak("$req not set"); }
	}
	for my $option(qw(InputFile OutputFile Break ResultsFile FailFile SuccessFile 
		Instructions Header Showoutput Stoponerror Skipsyntaxcheck Error)) {
		if($self->{$option}) { $args .= " " . $self->{$option}; } 
	}
	system($args)==0 or die("Command Manager script returned an error");  
};
...
$foo->add_attribute_child(	ATTRIBUTECHILD => "attributechild_name", 
...
$foo->add_attribute_child(	ATTRIBUTECHILD => "Day", 
...
$foo->add_attribute_form_expression(	ATTRIBUTEFORMEXP => "expression", 
...
Optional parameters: 	EXPSOURCETABLES => [ "<sourcetable1>" , "<sourcetable2>" , "<sourcetableN>"], 
			LOOKUPTABLE => "<lookup_table>", 
			OVERWRITE => "TRUE" | "FALSE"				  
...
$foo->add_attribute_form_expression(	ATTRIBUTEFORMEXP => "ORDER_DATE", 
...
$foo->add_attribute_form(	ATTRIBUTEFORM => "form_name", 
				FORMDESC => "form_description", 
				FORMCATEGORY => "category_name", 
				FORMTYPE => (NUMBER | TEXT | DATETIME | DATE | TIME | URL | EMAIL | HTML | PICTURE | BIGDECIMAL), SORT => (NONE | ASC | DESC), 
...
$foo->add_attribute_parent(ATTRIBUTEPARENT => "Month of Year", RELATIONSHIPTYPE => "MANYTOONE", ATTRIBUTE => "attribute_name", LOCATION => "location_path", PROJECT => "project_name");
...
$foo->add_configuration_ace(	CONF_OBJECT_TYPE => "conf_object_type", 
				...
				ACCESSRIGHTS_CUSTOM => { BROWSE => (GRANT | DENY), READ => (GRANT | DENY), WRITE => (GRANT | DENY), DELETE => (GRANT | DENY), CONTROL => (GRANT | DENY), USE  => (GRANT | DENY), EXECUTE => (GRANT | DENY) }
			   );
...
$foo->add_custom_group_element(	ELEMENT => "element_name", 
...
$foo->add_project_ace(PROJECT_OBJECT_TYPE => "project_object_type", OBJECT_NAME => "object_name", LOCATION => "location_path", (USER | GROUP) => (USER | GROUP), USER_LOGIN_OR_GROUP_NAME => "user_login_or_group_name", ACCESSRIGHTS => (VIEW | MODIFY | FULLCONTROL | DENIEDALL | DEFAULT | CUSTOM ), ACCESSRIGHTS_CUSTOM => { BROWSE => (GRANT | DENY), READ => (GRANT | DENY), WRITE => (GRANT | DENY), DELETE => (GRANT | DENY), CONTROL => (GRANT | DENY), USE  => (GRANT | DENY), EXECUTE => (GRANT | DENY) }, PROJECT => "project_name");
...
ALTER ACE FOR SCHEDULE "All the time" USER "Developer" ACCESSRIGHTS CUSTOM GRANT EXECUTE, USE, BROWSE, READ DENY WRITE, DELETE, CONTROL;
...
$foo->alter_connection_map(USER => "login_name", DBINSTANCE => "dbinstance_name", DBCONNECTION => "dbConnection_name", DBLOGIN => "dblogin_name", PROJECT => "project_name")
...
$foo->alter_connection_map(USER => "Developer", DBINSTANCE => "Tutorial Data", DBLOGIN => "Data", PROJECT => "MicroStrategy Tutorial");
...
$foo->alter_connection_map(USER => "jsmith", DBINSTANCE => "MSI_DB", DBCONNECTION => "MSI_DB_Conn", DBLOGIN => "MSI_USER", PROJECT => "MicroStrategy Tutorial");
...
$foo->alter_custom_group(	CUSTOMGROUP => "customgroup_name", 
				LOCATION => "location_path", 
...
Optional parameters: NEW_NAME => "<new_customgroup_name>", DESCRIPTION => "<new_description>", FOLDER => "<new_location_path>", ENABLEHIERARCHICALDISPLAY => (TRUE | FALSE), ENABLESUBTOTALDISPLAY => (TRUE | FALSE), ELEMENTHEADERPOSITION => (ABOVE | BELOW), HIDDEN => (TRUE | FALSE)
...
$foo->alter_custom_group(CUSTOMGROUP => "My Custom Groups", LOCATION => '\Public Objects\Custom Groups', NEW_NAME => "Modified My Custom Groups", NEW_DESCRIPTION => "Modified Copy of My Custom Groups", NEW_LOCATION => '\Public Objects\Custom Groups\Modified Custom Groups', ENABLEHIERARCHICALDISPLAY => "FALSE", ENABLESUBTOTALDISPLAY => "TRUE", ELEMENTHEADERPOSITION => "ABOVE", HIDDEN => "FALSE", PROJECT => "MicroStrategy Tutorial");
...
$foo->alter_custom_group(CUSTOMGROUP => "Copy of Age Groups", LOCATION => '\Public Objects\Custom Groups', NEW_NAME => "Modified Age Groups", NEW_DESCRIPTION => "Modified copy of Age Groups", ENABLEHIERARCHICALDISPLAY => "TRUE", ENABLESUBTOTALDISPLAY => "TRUE", ELEMENTHEADERPOSITION => "BELOW", HIDDEN => "TRUE", PROJECT => "MicroStrategy Tutorial");
...
$foo->alter_dbconnection(DBCONNECTION => "dbconnection_name", NEW_NAME => "new_dbconn_name", ODBCDSN => "new_odbc_datasource_name", DEFAULTLOGIN => "new_default_login", DRIVERMODE => (MULTIPROCESS | MULTITHREADED), EXECMODE => (SYNCHRONOUS | ASYNCHRONOUS (CONNECTION | STATEMENT)), USEEXTENDEDFETCH => (TRUE | FALSE), USEPARAMQUERIES => (TRUE | FALSE), MAXCANCELATTEMPT => "new_number_of_seconds", MAXQUERYEXEC => "new_number_of_seconds", MAXCONNATTEMPT => "new_number_of_seconds", CHARSETENCODING => (MULTIBYTE | UTF8), TIMEOUT => "new_number_of_seconds", IDLETIMEOUT => "new_number_of_seconds")
...
$foo->alter_dbconnection(DBCONNECTION => "DBConn1", NEW_NAME => "DBConnection2", ODBCDSN => "MSI_ODBC", DEFAULTLOGIN => "MSI_USER", DRIVERMODE => "MULTIPROCESS", EXECMODE => "SYNCHRONOUS", USEEXTENDEDFETCH => "TRUE", USEPARAMQUERIES => "TRUE", MAXCANCELATTEMPT => "100", MAXQUERYEXEC => "100", MAXCONNATTEMPT => "100", CHARSETENCODING => "MULTIBYTE", TIMEOUT => "100", IDLETIMEOUT => "100");
...
$foo->alter_dblogin(DBLOGIN => "dblogin_name", NEW_NAME => "new_dblogin_name", NEW_LOGIN => "new_database_login", NEW_PASSWORD => "new_database_pwd")
...
$foo->alter_dblogin(DBLOGIN => "MSI_USER", NEW_NAME => "MSI_USER2", NEW_LOGIN => "MSI_USER_login", NEW_PASSWORD => "resu_ism");

'ALTER DBLOGIN "Data" LOGIN "dbadmin" PASSWORD "dbadmin";

$foo->alter_dblogin(DBLOGIN => "Data", NEW_LOGIN => "dbadmin", NEW_PASSWORD => "dbadmin");
...
$foo->alter_element_caching(PROJECT => "project_name", MAXRAMUSAGE => "number_of_Kb", MAXRAMUSAGECLIENT => "number_of_kb", CREATECACHESPERDBLOGIN => (TRUE | FALSE), CREATECACHESPERDBCONN => (TRUE | FALSE))

Optional parameters: PROJECT => ,MAXRAMUSAGE => <number_of_Kb>,MAXRAMUSAGECLIENT => <number_of_kb>,CREATECACHESPERDBLOGIN => (TRUE | FALSE),CREATECACHESPERDBCONN => (TRUE | FALSE)
...
$foo->alter_element_caching(PROJECT => "MicroStrategy Tutorial", MAXRAMUSAGE => "10240", MAXRAMUSAGECLIENT => "512", CREATECACHESPERDBLOGIN => "TRUE", CREATECACHESPERDBCONN => "TRUE");

ALTER ELEMENT CACHING IN PROJECT "MicroStrategy Tutorial" MAXRAMUSAGE 10240 MAXRAMUSAGECLIENT 512;

$foo->alter_element_caching(PROJECT => "MicroStrategy Tutorial", MAXRAMUSAGE => "10240", MAXRAMUSAGECLIENT => "512");
...
$foo->alter_event(ALTER_EVENT => "event_name", NEW_NAME => "new_event_name", NEW_DESCRIPTION => "new_description")
...
$foo->alter_event(EVENT => "Database Load", NEW_NAME => "DBMS_Load", NEW_DESCRIPTION => "Modified Database Load");
...
$foo->alter_fact(ALTER_FACT => "fact_name", LOCATION => "location_path", NAME => "new_fact_name", DESCRIPTION => "new_description", FOLDER => "new_location_path", HIDDEN => (TRUE | FALSE), PROJECT => "project_name")

Optional parameters: NEW_NAME => "<new_fact_name>", NEW_DESCRIPTION => "<new_description>", NEW_LOCATION => "<new_location_path>", HIDDEN => (TRUE | FALSE)
...
$foo->alter_fact(ALTER_FACT => "Revenue", LOCATION => 'Public Objects', NAME => "Copy Revenue", DESCRIPTION => "Altered Revenue", FOLDER => '\Project Objects', HIDDEN => "TRUE", PROJECT => "MicroStrategy Tutorial");
...
$foo->alter_filter(FILTER => "filter_name", LOCATION => "location_path", NEW_NAME => "new_filter_name", NEW_EXPRESSION => "new_expression", NEW_DESCRIPTION => "new_description", NEW_LOCATION => "new_location_path", HIDDEN => (TRUE | FALSE), PROJECT => "project_name");
...
$foo->alter_filter(FILTER => "South Region", LOCATION => '\Public Objects\Filters', NEW_NAME => "Southeast Region", NEW_EXPRESSION => 'Region@ID=3', NEW_DESCRIPTION => "Modified South Region filter", NEW_LOCATION => '\Public Objects\Filters\South Region', HIDDEN => "FALSE", PROJECT => "MicroStrategy Tutorial");

ALTER FILTER "On Promotion(CM)" IN "\Public Objects\Filters" HIDDEN FALSE ON PROJECT "MicroStrategy Tutorial";

$foo->alter_filter(FILTER => 'On Promotion(CM)', LOCATION => '\Public Objects\Filters', HIDDEN => "FALSE", PROJECT => "MicroStrategy Tutorial");
...
$foo->alter_filter(FILTER => "West Region", LOCATION => 'Public Objects\Filters', NEW_NAME => "East Region", NEW_EXPRESSION => 'Region@ID=2', PROJECT => "MicroStrategy Tutorial");
...
$foo->alter_folder_ace(FOLDER => "folder_name", LOCATION => "location_path", USER_OR_GROUP => (USER | GROUP), USER_LOGIN_OR_GROUP_NAME => "user_login_or_group_name", ACCESSRIGHTS => (VIEW | MODIFY | FULLCONTROL | DENIEDALL | DEFAULT | CUSTOM ), ACCESSRIGHTS_CUSTOM => { BROWSE => (GRANT | DENY), READ => (GRANT | DENY), WRITE => (GRANT | DENY), DELETE => (GRANT | DENY), CONTROL => (GRANT | DENY), USE  => (GRANT | DENY), EXECUTE => (GRANT | DENY) }, CHILDRENACCESSRIGHTS => (VIEW | MODIFY | FULLCONTROL | DENIEDALL | DEFAULT | CUSTOM ), CHILDRENACCESSRIGHTS_CUSTOM => { BROWSE => (GRANT | DENY), READ => (GRANT | DENY), WRITE => (GRANT | DENY), DELETE => (GRANT | DENY), CONTROL => (GRANT | DENY), USE  => (GRANT | DENY), EXECUTE => (GRANT | DENY) }, PROJECT => "project_name");

Optional parameters: ACCESSRIGHTS_CUSTOM, CHILDRENACESSRIGHTS_CUSTOM

ALTER ACE FOR FOLDER "Subtotals" IN FOLDER "\Project Objects" USER "Developer" ACCESSRIGHTS CUSTOM DENY BROWSE, READ CHILDRENACCESSRIGHTS CUSTOM GRANT BROWSE, READ FOR PROJECT "MicroStrategy Tutorial";
...
$foo->alter_folder_acl(ALTER_ACL_FOR_FOLDER => "folder_name", LOCATION => "location_path", PROPAGATE_OVERWRITE_RECURSIVELY_PROJECT => "project_name")

Optional parameters: PROPAGATE => OVERWRITE RECURSIVELY
...
$foo-alter_folder(FOLDER => "folder_name", LOCATION => "location_path", NEW_NAME => "new_folder_name", NEW_DESCRIPTION => "new_description", HIDDEN => (TRUE | FALSE), NEW_LOCATION => "new_location_path", PROJECT => "project_name");
...
$foo->alter_metric(METRIC => "metric_name", LOCATION => "location_path", NEW_NAME => "new_metric_name", NEW_EXPRESSION => "new_expression", NEW_DESCRIPTION => "new_description", NEW_LOCATION => "new_location_path", HIDDEN => (TRUE | FALSE), PROJECT => "project_name");

Optional parameters: FOLDER => ,NAME => "<new_metric_name>",EXPRESSION => "<new_expression>",DESCRIPTION => "<new_description>",LOCATION => "<new_location_path>",HIDDEN => (TRUE | FALSE)
...
ALTER METRIC "New Metric" IN FOLDER "\Public Objects\Metrics\Count Metrics" LOCATION "\Public Objects\Metrics" ON PROJECT "MicroStrategy Tutorial";

ALTER METRIC "New Metric" IN FOLDER "\Public Objects\Metrics\Count Metrics" LOCATION "\Public Objects\Metrics\Count Metrics" HIDDEN FALSE ON PROJECT "MicroStrategy Tutorial";
...
$foo->alter_object_caching(PROJECT => "project_name", MAXRAMUSAGE => "number_of_Kb", MAXRAMUSAGECLIENT => "number_of_kb")
...
$foo->alter_project_ace(ALTER_ACE_FOR => "project_object_type", "OBJECT_NAME" => "object_name", LOCATION => "location_path", (USER | GROUP) => (USER | GROUP), "USER_LOGIN_OR_GROUP_NAME" => "user_login_or_group_name", ACCESSRIGHTS => (VIEW | MODIFY | FULLCONTROL | DENIEDALL | DEFAULT | CUSTOM [GRANT "accessright1" [, "accessright2" [,... "accessrightN"]]] [DENY "accessright1" [, "accessright2" [,... "accessrightN"]]] [DEFAULT accessright1 [, accessright2 [, accessrightn]]]), PROJECT => "project_name")

Optional parameters: USER_OR_GROUP => (USER | GROUP), USER_LOGIN_OR_GROUP_NAME => "<user_login_or_group_name>", ACCESSRIGHTS (VIEW | MODIFY | FULLCONTROL | DENIEDALL | DEFAULT | CUSTOM GRANT <accessright1> , <accessright2> ,... <accessrightN> DENY <accessright1> , <accessright2> ,... <accessrightN> DEFAULT accessright1 , accessright2 , accessrightn)
...
$foo->alter_project_config(	DESCRIPTION => "Project_description", 
				WAREHOUSE => "WH_name", 
...
Optional parameters: DESCRIPTION => "<Project_description>",
			WAREHOUSE => "<WH_name>",
			STATUS => "<html_input_file>",
			SHOWSTATUS => (TRUE | FALSE),
			STATUSONTOP => (TRUE | FALSE),
...
ALTER PROJECT CONFIGURATION DESCRIPTION "MicroStrategy Tutorial Project (version 7.2)" WAREHOUSE "Tutorial Data" DOCDIRECTORY "C:\Program Files\MicroStrategy\Tutorial Reporting" MAXNOATTRELEMS 1000 USEWHLOGINEXEC FALSE MAXREPORTEXECTIME 600 MAXNOREPORTRESULTROWS 32000 MAXNOELEMROWS 32000 MAXNOINTRESULTROWS 32000 MAXJOBSUSERACCT 100 MAXJOBSUSERSESSION 100 MAXEXECJOBSUSER 100 MAXJOBSPROJECT 1000 MAXUSERSESSIONSPROJECT 500 IN PROJECT "MicroStrategy Tutorial";

ALTER PROJECT CONFIGURATION PROJDRILLMAP "\Public Objects\Drill Maps\MicroStrategy Tutorial Drill Map" REPORTTPL "Blank Report" REPORTSHOWEMPTYTPL FALSE TEMPLATETPL "Default Template" TEMPLATESHOWEMPTYTPL FALSE METRICTPL "Default Metric" METRICSHOWEMPTYTPL FALSE IN PROJECT "MicroStrategy Tutorial";
...
for(@selected) {
	/DESCRIPTION/ && do { $result .= "ALTER PROJECT CONFIGURATION DESCRIPTION " . $q . $self->{DESCRIPTION} . $q . " "};
...
$foo->alter_report_caching(	PROJECT => "project_name", 
...
Optional parameters: 		ENABLED => (ENABLED | DISABLED),  
				CACHEFILEDIR => "cache_file_directory", 
				MAXRAMUSAGE => "number_of_Kb", 
				MAXNOCACHES => "number_of_Caches", 
				LOADCACHESONSTARTUP => (TRUE | FALSE), 
...
$foo->alter_report(	REPORT => "report_name", 
			LOCATION => "location_path", 
...
Optional parameters: 	ENABLECACHE => (TRUE | FALSE | DEFAULT),
			NEW_NAME => "new_report_name", 
			NEW_LONGDESCRIPTION => "new_long_description", 
			NEW_DESCRIPTION => "new_description", 
			NEW_LOCATION => "new_location_path", 
...
ALTER REPORT 'Profit Forecast' IN FOLDER '\Public Objects\Reports\Subject Areas\Enterprise Performance Management' ENABLECACHE TRUE FOR PROJECT 'MicroStrategy Tutorial';

ALTER REPORT 'Profit Forecast' IN FOLDER '\Public Objects\Reports\Subject Areas\Enterprise Performance Management' NAME "Copy of Profit Forecast" DESCRIPTION "This report contains the profit forecast for the year 2004" ENABLECACHE TRUE FOR PROJECT 'MicroStrategy Tutorial';

ALTER REPORT 'Profit Forecast' IN FOLDER '\Public Objects\Reports\Subject Areas\Enterprise Performance Management' HIDDEN TRUE FOR PROJECT 'MicroStrategy Tutorial';

ALTER REPORT 'Profit Forecast' IN FOLDER '\Public Objects\Reports\Subject Areas\Enterprise Performance Management' HIDDEN FALSE FOR PROJECT 'MicroStrategy Tutorial';

ALTER REPORT 'Profit Forecast' IN FOLDER '\Public Objects\Reports\Subject Areas\Enterprise Performance Management' LONGDESCRIPTION "Profit forecast report" FOR PROJECT 'MicroStrategy Tutorial';
...
$foo->alter_schedule(	SCHEDULE => "schedule_name",
			NEW_NAME => "new_name", 
			DESCRIPTION => "new_description",
			...
			WEEKLY => "EVERY <new_number> WEEKS ON day_of_week1 [, day_of_week2 [,... day_of_week7]]",
			MONTHLY => "(DAY <new_number> OF EVERY <new_number> MONTHS (FIRST | SECOND | THIRD | FOURTH | LAST) (day_of_week1 | day_of_week2 | ... | day_of_week7) OF EVERY <new_number> MONTHS)",
			YEARLY => "((month_of_year1 | month_of_year2 | ... | month_of_year12) <new_number> 
(FIRST | SECOND | THIRD | FOURTH | LAST) (day_of_week1 | day_of_week2 | ... | day_of_week7) OF (month_of_year1 | month_of_year2 | ... | month_of_year12))"
			EXECUTE_TIME_OF_DAY => new_time_of_day, 
			EXECUTE_ALL_DAY => "EVERY <new_number> (MINUTES | HOURS [START AFTER MIDNIGHT <number> MINUTES] )",
		   );
...
$foo->alter_schedule(	SCHEDULE => "Database Load",
			STARTDATE => '09/10/2002',
			ENDDATE => "NEVER",
			TYPE => "EVENTTRIGGERED",
...
$foo->alter_schedule(	SCHEDULE => "Schedule3",
			STARTDATE => '09/10/2002',
			ENDDATE => "NEVER",
			TYPE => "TIMETRIGGERED",
			DAILY => "EVERY 5 DAYS",
...
$foo->alter_schedule(	SCHEDULE => "Schedule4",
			STARTDATE => '09/10/2002',
			ENDDATE =>  "NEVER",
			TYPE => "TIMETRIGGERED",
			DAILY => "EVERY WEEKDAY",
...
$foo->alter_schedule(	SCHEDULE => "Schedule5",
			STARTDATE => '09/10/2002',
			ENDDATE =>  "NEVER",
			TYPE => "TIMETRIGGERED",
...
$foo->alter_schedule(	SCHEDULE => "Schedule6",
			STARTDATE => '09/10/2002',
			ENDDATE =>  '09/27/02',
			TYPE => "TIMETRIGGERED",
...
$foo->alter_schedule(	SCHEDULE => "Schedule7",
			STARTDATE => '09/10/2002',
			ENDDATE =>  "NEVER",
			TYPE => "TIMETRIGGERED",
...
$foo->alter_schedule(	SCHEDULE => "Schedule8",
			STARTDATE => '09/10/2002',
			ENDDATE =>  "NEVER",
			TYPE => "TIMETRIGGERED",
			YEARLY => "MARCH 10",
...
$foo->alter_schedule(	SCHEDULE => "Schedule9",
			STARTDATE => "09/10/2002",
			ENDDATE => "NEVER",
			TYPE => "TIMETRIGGERED",
...
$foo->alter_security_filter(	SECURITY_FILTER => "sec_filter_name", 
			     	LOCATION => "location_path", 
				HIDDEN => (TRUE | FALSE), 
				PROJECT => "project_name", 
...
Optional parameters: FOLDER => "<location_path>",HIDDEN => (TRUE | FALSE),PROJECT => ,NAME => "<new_sec_filter_name>",(FILTER => "<filter_name>" IN FOLDER "<filter_location_path>" | EXPRESSION "<new_expression>"),TOP => ATTRIBUTE LIST "<top_attr_name1>" , "<top_attr_name2>" , ... "<top_attr_nameN>",BOTTOM => ATTRIBUTE LIST "<bottom_attr_name1>" , "<bottom_attr_name2>" , ... "<bottom_attr_nameN>"
...
ALTER SECURITY FILTER "SecFilter1" IN "MicroStrategy Tutorial" FILTER "Western United States Customers";

ALTER SECURITY FILTER "SecFilter2" IN PROJECT "MicroStrategy Tutorial" EXPRESSION "Region@ID=3";

ALTER SECURITY FILTER "SecFilter4" HIDDEN FALSE IN PROJECT "MicroStrategy Tutorial" EXPRESSION "Region@ID=3";
...
$foo->alter_security_role(	SECURITY_ROLE => "sec_role_name", 
				NAME => "new_sec_role_name", 
				DESCRIPTION => "sec_role_description"
			 );

Optional parameters: NAME => "<new_sec_role_name>",DESCRIPTION => 
"<sec_role_description>"
...
$foo->alter_server_config(	DESCRIPTION => "description", 
...
$foo->alter_shortcut(	LOCATION => "location_path", 
			PROJECT_CONFIG_OBJECT => (FOLDER | CONSOLIDATION | DOCUMENT | FILTER | METRIC | PROMPT | REPORT | SEARCH | TEMPLATE | ATTRIBUTE | FACT | FUNCTION | HIERARCHY | TABLE | TRANSFORMATION | DRILLMAP | SECFILTER | AUTOSTYLE | BASEFORMULA), 
...
ALTER SHORTCUT IN FOLDER "\Public Objects" FOR FOLDER "Drill Maps" FOLDER "\Project Objects" HIDDEN FALSE FOR PROJECT "MicroStrategy Tutorial";

ALTER SHORTCUT IN FOLDER "\Project Objects" FOR FOLDER "Drill Maps" HIDDEN TRUE FOR PROJECT "MicroStrategy Tutorial";
...
$foo->alter_statistics(		DBINSTANCE => "stats_dbinstance", 
				ENABLED => (ENABLED | DISABLED), 
...
Optional parameters: DBINSTANCE => "<stats_dbinstance>",USERSESSIONS => (TRUE | FALSE),PROJECTSESSIONS => (TRUE | FALSE),BASICDOCJOBS => (TRUE | FALSE),BASICREPJOBS => (TRUE | FALSE),CACHES => (TRUE | FALSE),SCHEDULES => 
(TRUE | FALSE),COLUMNSTABLES => (TRUE | FALSE),DETAILEDREPJOBS => (TRUE | FALSE),JOBSQL => (TRUE | FALSE),SECFILTERS => (TRUE | FALSE)
...
$foo->alter_statistics(		DBINSTANCE => "Tutorial Data", 
				ENABLED => "ENABLED", 
				USERSESSIONS => "TRUE", 
...
$foo->alter_table(	TABLE => "table_name", 
...
Optional parameters: NAME => "<new_table_name>",DESCRIPTION => "<new_description>",FOLDER => "<new_location_path>",HIDDEN => (TRUE | FALSE),LOGICALSIZE => <logical_size>,PRESERVELOGICALSIZE => (TRUE | FALSE)
...
$foo->alter_table(	TABLE => "DT_QUARTER", 
			NEW_NAME => "2", 
			NEW_DESCRIPTION => "1", 
...
$foo->alter_user_group(		USER_GROUP => "user_group_name", 
...
$foo->alter_user(	USER => "login_name", 
			NAME => "new_login_name", 
			NTLINK => "nt_user_id", 
...
$foo->alter_users(	USER_GROUP => "user_group_name", 
...
Optional parameters: PASSWORD => "<new_password>",DESCRIPTION
"<new_user_description>" => ,ALLOWCHANGEPWD => (TRUE | FALSE),ALLOWSTDAUTH => (TRUE | FALSE),CHANGEPWD => (TRUE | FALSE),PASSWORDEXP => (NEVER | IN <new_number_of_days> DAYS |ON 
<new_expiration_date>,PASSWORDEXPFREQ => <number> DAYS,(ENABLED => | DISABLED,GROUP => "<new_user_group>"
...
$foo->apply_security_filter(	SECURITY_FILTER => "sec_filter_name", 
				LOCATION => "location_path", 
				USER_OR_GROUP => ( USER | GROUP), 
...
$foo->create_attribute(	ATTRIBUTE => "attribute_name", 
			DESCRIPTION => "description", 
			LOCATION => "location_path", 
			HIDDEN => ( TRUE| FALSE), 
			ATTRIBUTEFORM => "form_name", 
			FORMDESC => "form_description", 
			FORMTYPE => (NUMBER | TEXT | DATETIME | DATE | TIME | URL | EMAIL | HTML | PICTURE | BIGDECIMAL), 
			SORT => (NONE | ASC | DESC), 
			EXPRESSION => "form_expression", 
...
Optional parameters: 	DESCRIPTION => "<description>",
			...
			FORMTYPE => (NUMBER | TEXT | DATETIME | DATE | TIME | URL | EMAIL | HTML | PICTURE | BIGDECIMAL),
			SORT => (NONE | ASC | DESC),
			EXPSOURCETABLES => ["<sourcetable1>" , "<sourcetable1> , ..."<sourcetableN>"]
...
$foo->create_connection_map(	USER => "login_name", 
...
CREATE CONNECTION MAP FOR USER "Developer" DBINSTANCE "Tutorial Data" DBCONNECTION "Data" DBLOGIN "Data" ON PROJECT "MicroStrategy Tutorial";
...
$foo->create_custom_group(	CUSTOMGROUP => "customgroup_name", 
				DESCRIPTION => "description", 
				ENABLEHIERARCHICALDISPLAY => "TRUE" | "FALSE", 
				ENABLESUBTOTALDISPLAY => "TRUE" | "FALSE",  
				ELEMENTHEADERPOSITION => "ABOVE" | "BELOW", 
...
Optional parameters: DESCRIPTION => "<description>",ENABLEHIERARCHICALDISPLAY => (TRUE | FALSE),ENABLESUBTOTALDISPLAY => (TRUE | FALSE),ELEMENTHEADERPOSITION => (ABOVE | BELOW),HIDDEN => (TRUE | FALSE),(SHOWELEMENTNAME =>  | SHOWITEMSINELEMENT | SHOWITEMSINELEMENTANDEXPAND  | SHOWALLANDEXPAND),BREAKAMBIGUITY => FOLDER "<local_symbol_folder>",BANDNAMES => "<name1>", "<name2>", "<nameN>",OUTPUTLEVEL =>  "<attribute_name1>", "<attribute_name2>", "<attributenameN>"  IN FOLDERS "<outputlevel_location_path1>", "<outputlevel_location_path2>", "<outputlevel_location_pathN>"
...
CREATE CUSTOMGROUP "Copy of Age Groups" DESCRIPTION "Clone of Age Groups" ENABLEHIERARCHICALDISPLAY TRUE ENABLESUBTOTALDISPLAY TRUE ELEMENTHEADERPOSITION ABOVE  ELEMENT "<25" SHOWELEMENTNAME EXPRESSION "([Customer Age]@ID < 25.0)" IN FOLDER "\Public Objects\Custom Groups" FOR PROJECT "MicroStrategy Tutorial";

CREATE CUSTOMGROUP "Copy of Custom Categories" DESCRIPTION "Clone of Custom Categories" ENABLEHIERARCHICALDISPLAY TRUE ENABLESUBTOTALDISPLAY TRUE  ELEMENTHEADERPOSITION ABOVE ELEMENT "Category Sales" SHOWELEMENTNAME EXPRESSION "(Category@Desc In (^"Books^",^"Electronics^",^"Movies^",^"Music^"))" IN FOLDER "\Public Objects\Custom Groups"  FOR PROJECT "MicroStrategy Tutorial";
CREATE CUSTOMGROUP "Copy of Customers Deciling" DESCRIPTION "Clone of Customer Deciling" ENABLEHIERARCHICALDISPLAY TRUE ELEMENTHEADERPOSITION ABOVE ELEMENT "All  Customers" SHOWELEMENTNAME EXPRESSION "BandingP(Rank<ByValue=False>([\Public Objects\Metrics\Sales Metrics\Revenue]),0,0.1,0.5,1)" BANDNAMES "Top 10% of Customers","Next 40%  OF Customers", "Bottom 50% of Customers" OUTPUTLEVEL "Customer" IN FOLDERS "\Schema Objects\Attributes\Customers" IN FOLDER "\Public Objects\Custom Groups\" FOR PROJECT  "MicroStrategy Tutorial";

CREATE CUSTOMGROUP "Copy of Customers Value Banding" DESCRIPTION "Clone of Customers Value Banding" ENABLEHIERARCHICALDISPLAY TRUE ELEMENTHEADERPOSITION ABOVE ELEMENT  "All Customers" SHOWELEMENTNAME EXPRESSION "Banding([Running Revenue Contribution to All Customers Abs.],0.0,1.0,0.2)" BANDNAMES "Top Quintile", "Second Quintile", "Third Quintile",  "Fourth Quintile", "Bottom Quintile" OUTPUTLEVEL "Customer" IN FOLDERS "\Schema Objects\Attributes\Customers" IN FOLDER "\Public Objects\Custom Groups\" FOR PROJECT "MicroStrategy Tutorial";

CREATE CUSTOMGROUP "Copy of Top 5 Contributors vs. All" ELEMENT "Top 5 Customers by Revenue" EXPRESSION "([Top 5 Customers by Revenue])" IN FOLDER "\Public Objects\Custom Groups\" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->create_dbconnection(	DBCONNECTION => "dbconnection_name", 
...
CREATE DBCONNECTION "DBConn1" ODBCDSN "SQLSERVER_WH" DEFAULTLOGIN "Data" DRIVERMODE MULTIPROCESS EXECMODE ASYNCHRONOUS STATEMENT MAXCANCELATTEMPT 10 MAXQUERYEXEC 20 TIMEOUT 50 IDLETIMEOUT 100;
...
$foo->create_dbinstance(	DBINSTANCE => "dbinstance_name", 
...
Optional parameters: DESCRIPTION => "<description>",DATABASE => "<database_name>",TABLESPACE => "<tablespace_name>",PRIMARYDBINSTANCE => "<dbinstance_name>",DATAMART => 
...
CREATE DBINSTANCE "Production Database" DBCONNTYPE "Microsoft SQL Server 2000" DBCONNECTION "Data" DESCRIPTION "Production database" HIGHTHREADS 10 MEDIUMTHREADS 5 LOWTHREADS 7;
...
$foo->create_dblogin(	DBLOGIN => "dblogin_name", 
...
CREATE DBLOGIN "Data" LOGIN "dbadmin" PASSWORD "dbadmin";
...
$foo->create_event(	EVENT => "event_name", 
			DESCRIPTION => "description"
		  );
...
$foo->create_fact(	FACT => "fact_name", 
			DESCRIPTION => "description", 
			LOCATION => "location_path", 
			HIDDEN => (TRUE | FALSE), 
			EXPRESSION => "expression", 
...
Optional parameters: DESCRIPTION => "<description>",HIDDEN => (TRUE | FALSE),EXPSOURCETABLES => "<sourcetable1>" , "<sourcetable2>" , "<sourcetableN>"
...
CREATE FACT "Profit" DESCRIPTION "A modified copy of Profit" IN FOLDER "\Public Objects" EXPRESSION "TOT_DOLLAR_SALES - TOT_COST" FOR PROJECT "Microstrategy Tutorial";

CREATE FACT "Revenue" DESCRIPTION "A copy of Revenue" IN FOLDER "\Public Objects" EXPRESSION "TOT_DOLLAR_SALES" FOR PROJECT "Microstrategy Tutorial";
...
$foo->create_filter_oultine(	FILTER => "filter_name", 
				LOCATION => "location_path", 
				EXPRESSION => "expression", 
				DESCRIPTION => "description", 
				HIDDEN => (TRUE | FALSE), 
...
CREATE FILTER "West Region" IN FOLDER "\Public Objects\Filters" EXPRESSION "Region@ID=1" ON PROJECT "MicroStrategy Tutorial";

CREATE FILTER "RegionNorthEast" IN FOLDER "\Public Objects\Filters" EXPRESSION  "Region@DESC in ("North-East")" ON PROJECT "MicroStrategy Tutorial";

CREATE FILTER "On Promotion(CM)" IN FOLDER "\Public Objects\Filters" EXPRESSION "Promotion@DESC in ("Valentine's Day", "Summer Fun")" HIDDEN TRUE ON PROJECT "MicroStrategy Tutorial";
...
$foo->create_folder(	FOLDER => "folder_name", 
			LOCATION => "location_path", 
			DESCRIPTION => "description", 
			HIDDEN => (TRUE | FALSE), 
			PROJECT => "project_name"
...
$foo->create_metric_oultine(	METRIC => "metric_name", 
				LOCATION => "location_path", 
				EXPRESSION => "expression", 
				DESCRIPTION => "description", 
				HIDDEN => (TRUE | FALSE), 
...
CREATE METRIC "New Metric" IN FOLDER "\Public Objects\Metrics\Count Metrics" EXPRESSION "Count(Customer) {Country} <[Western United States Customers]>" ON PROJECT "MicroStrategy Tutorial";

CREATE METRIC "ApplySimple Metric Sample" IN FOLDER "\Public Objects\Metrics\Sales Metrics" EXPRESSION 'ApplySimple("#0*2", Sum([Item inventory]) {~+} )'  ON PROJECT "MicroStrategy Tutorial";

CREATE METRIC "New Metric" IN FOLDER "\Public Objects\Metrics\Count Metrics" EXPRESSION "Count(Customer) {Country}" HIDDEN TRUE ON PROJECT "MicroStrategy Tutorial";
...
$foo->create_schedule(	SCHEDULE => "schedule_name", 
			NEW_NAME => "new_name", 
			DESCRIPTION => "new_description",
			STARTDATE => new_start_date,
			ENDDATE => (new_end_date | "NEVER"),
			...
			WEEKLY => "EVERY <new_number> WEEKS ON day_of_week1 [, day_of_week2 [,... day_of_week7]]",
			MONTHLY => "(DAY <new_number> OF EVERY <new_number> MONTHS (FIRST | SECOND | THIRD | FOURTH | LAST) (day_of_week1 | day_of_week2 | ... | day_of_week7) OF EVERY <new_number> MONTHS)",
			YEARLY => "((month_of_year1 | month_of_year2 | ... | month_of_year12) <new_number> (FIRST | SECOND | THIRD | FOURTH | LAST) (day_of_week1 | day_of_week2 | ... | day_of_week7) OF (month_of_year1 | month_of_year2 | ... | month_of_year12))",
			EXECUTE_TIME_OF_DAY => new_time_of_day, 
			EXECUTE_ALL_DAY => "EVERY <new_number> (MINUTES | HOURS [START AFTER MIDNIGHT <number> MINUTES] )",
     );
...
CREATE SCHEDULE "Database Load" STARTDATE 09/10/2002 ENDDATE NEVER TYPE EVENTTRIGGERED EVENTNAME "Database Load";
CREATE SCHEDULE "Schedule1" STARTDATE 09/10/2002 ENDDATE NEVER TYPE TIMETRIGGERED DAILY EVERY 5 DAYS EXECUTE 10:00;
CREATE SCHEDULE "Schedule2" STARTDATE 09/10/2002 ENDDATE NEVER TYPE TIMETRIGGERED DAILY EVERY WEEKDAY EXECUTE ALL DAY EVERY 5 MINUTES;
CREATE SCHEDULE "Schedule3" STARTDATE 09/10/2002 ENDDATE NEVER TYPE TIMETRIGGERED WEEKLY EVERY 5 WEEKS ON MONDAY, TUESDAY, WEDNESDAY EXECUTE 18:00;
CREATE SCHEDULE "Schedule4" STARTDATE 09/10/2002 ENDDATE 09/27/02 TYPE TIMETRIGGERED MONTHLY DAY 3 OF EVERY 5 MONTHS EXECUTE ALL DAY EVERY 5 HOURS START AFTER MIDNIGHT 10 MINUTES;
LIST PROPERTIES FOR SCHEDULE "Schedule1";
DELETE SCHEDULE "Schedule4";
CREATE SCHEDULE "Schedule5" STARTDATE 09/10/2002 ENDDATE NEVER TYPE TIMETRIGGERED MONTHLY FIRST THURSDAY OF EVERY 10 MONTHS EXECUTE 13:00;
CREATE SCHEDULE "Schedule6" STARTDATE 09/10/2002 ENDDATE NEVER TYPE TIMETRIGGERED YEARLY MARCH 10 EXECUTE 17:00;
CREATE SCHEDULE "Schedule7" STARTDATE 09/10/2002 ENDDATE NEVER TYPE TIMETRIGGERED YEARLY SECOND SATURDAY OF MAY EXECUTE 09:00;
...
$foo->create_schedule_relation(	SCHEDULE => "schedule_name", 
				USER_OR_GROUP => (USER | GROUP), 
				USER_LOGIN_OR_GROUP_NAME => "user_login_or_group_name", 
...
CREATE SCHEDULERELATION SCHEDULE "Monday Morning" USER "crosie" REPORT "Revenue vs. Forecast" IN "\Public Objects\Reports\Subject Areas\Sales and Profitability Analysis" IN PROJECT "MicroStrategy Tutorial";

CREATE SCHEDULERELATION SCHEDULE "All the Time" USER "rnyderman" REPORT "Electronics Sales Over Time" IN "\Public Objects\Reports\Subject Areas\Sales and Profitability Analysis" IN PROJECT "MicroStrategy Tutorial";
...
$foo->create_security_filter(	SECURITY_FILTER => "sec_filter_name", 
				LOCATION => "location_path", 
				HIDDEN => "TRUE" | "FALSE",
				PROJECT => "project_name", 
				FILTER => "FILTER_NAME",
...
CREATE SECURITY FILTER "SecFilter1" IN PROJECT "MicroStrategy Tutorial" FILTER "Eastern United States Customers" TOP ATTRIBUTE LIST "Year" BOTTOM ATTRIBUTE LIST "Region", "Salary";

CREATE SECURITY FILTER "SecFilter3" FOLDER "\Project Objects\MD Security Filters" IN PROJECT "MicroStrategy Tutorial" FILTER "Western United States Customers";

CREATE SECURITY FILTER "SecFilter2" IN PROJECT "MicroStrategy Tutorial" EXPRESSION "Region@ID=1";

CREATE SECURITY FILTER "SecFilter4" HIDDEN TRUE IN PROJECT "MicroStrategy Tutorial" EXPRESSION "Region@ID=1";
...
$foo->create_security_role(	SECURITY_ROLE => "sec_role_name", 
				DESCRIPTION => "sec_role_description"
			  );
...
CREATE SECURITY ROLE "Architect Users" DESCRIPTION "Sec Role for Architects";
...
$foo->create_shortcut(	SHORTCUT_LOCATION => "location_path", 
			PROJECT_CONFIG_OJECT => (FOLDER | CONSOLIDATION | DOCUMENT | FILTER | METRIC | PROMPT | REPORT | SEARCH | TEMPLATE | ATTRIBUTE | FACT | FUNCTION | HIERARCHY | TABLE | TRANSFORMATION | DRILLMAP | SECFILTER | AUTOSTYLE | BASEFORMULA), 
			OBJECT_NAME" => "object_name", 
...
$foo->create_user_group(	USER_GROUP => "user_group_name", 
				DESCRIPTION => "user_group_desc", 
				LDAPLINK => "ldap_user_id", 
				MEMBERS => ["login_name1", "login_nameN"], 
...
Optional parameters: DESCRIPTION => "<user_group_desc>",LDAPLINK => "<ldap_user_id>",MEMBERS => "<login_name1>" , "<login_name2>" ,... "<login_nameN>",IN => GROUP "<parent_user_group_name>"
...
CREATE USER GROUP "Managers" MEMBERS "Developer","User";
...
$foo->create_user(	USER => "login_name" | "nt_user_id",
...
This sample creates two users with different properties. 
...
CREATE USER IMPORTWINUSER "\DomainName\UserName" ALLOWCHANGEPWD TRUE ALLOWSTDAUTH TRUE;
...
$foo->create_user_profile(	USER => "login_name", 
				LOCATION => "location_path", 
				PROJECT => "project_name"
			 );
...
CREATE USER PROFILE FOR USER "mroe" FOR PROJECT "MicroStrategy Tutorial";

CREATE PROFILE FOR "Architect" FOR PROJECT "MicroStrategy Tutorial";

CREATE USER PROFILE FOR USER "baoter" IN FOLDER "\Public Objects" FOR PROJECT "Microstrategy Tutorial";
...
DELETE ATTRIBUTE "Duplicate_Day" IN FOLDER "\Schema Objects\Attributes\Time" FOR PROJECT "MicroStrategy Tutorial";
...
DELETE CONNECTION MAP FOR USER "Developer" DBINSTANCE "Tutorial Data" ON PROJECT "MicroStrategy Tutorial";
...
DELETE CUSTOMGROUP "Modified Age Groups" IN FOLDER "\Public Objects\Custom Groups" FROM PROJECT "MicroStrategy Tutorial";
...
$foo->delete_dblogin(DELETE_DBLOGIN => "dblogin_name")
...
DELETE FACT "Copy Profit" IN FOLDER "\Project Objects" FOR PROJECT "MicroStrategy Tutorial";
DELETE FACT "Copy Revenue" IN FOLDER "\Project Objects" FOR PROJECT "MicroStrategy Tutorial";
...
Optional parameters: FOLDER => 

DELETE FILTER "<filter_name>" IN [FOLDER] "<location_path>" FROM PROJECT "<project_name>";

DELETE FILTER "East Region" IN FOLDER "\Public Objects\Filters" FROM PROJECT "MicroStrategy Tutorial";
...
DELETE METRIC "New Metric" IN FOLDER "\Public Objects\Metrics\Count Metrics" FROM PROJECT "MicroStrategy Tutorial";
...
$foo->delete_report_cache( 	INVALID => TRUE | FALSE,
...
DELETE REPORT "Quarterly Profit Margins" IN FOLDER "\Public Objects\Reports\Subject Areas\Enterprise Performance Management" FROM PROJECT "MicroStrategy Tutorial";
...
$foo->delete_schedule_relation(	DELETE_MULTIPLE => "TRUE" | "FALSE",
				USER_OR_GROUP => "USER" | "GROUP",
...
Optional parameters: 		USER_OR_GROUP => "USER" | "GROUP",
				USER_LOGIN_OR_GROUP_NAME =>  "user_login_or_group_name",
				REPORT => "rep_or_doc_name" 
				LOCATION => "location_path" 
				SCHEDULE => "schedule_name"
...
Optional parameters: FOLDER => "<location_path>"

DELETE SECURITY FILTER "<sec_filter_name>" [FOLDER "<location_path>"] FROM PROJECT "<project_name>";

DELETE SECURITY FILTER "SecFilter1" FROM PROJECT "MicroStrategy Tutorial";
...
$foo->delete_shortcut(	LOCATION => '\Project Objects', 
...
$foo->disconnect_database_connection( CONNECTION => "ALL" | "connection_id");
...
$foo->disconnect_user(	USER => "login_name" ,
			SESSIONID => <sessionid>
			PROJECT => "project_name",
...
$foo->disconnect_user(	PROJECT => "project_name",
			ALL_SESSIONS => "TRUE" );

DISCONNECT ALL USER SESSIONS;

$foo->disconnect_user(ALL_SESSIONS => "TRUE");
...
$foo->expire_report_cache(	REPORT_CACHE => "ALL" | "cache_name", 
				PROJECT => "project_name"
			 );
...
$foo->get_attribute_child_candidates(	ATTRIBUTE => "attribute_name", 
					LOCATION => "location_path", 
					PROJECT => "project_name"
				    );
...
GET CHILD CANDIDATES FOR ATTRIBUTE "Month" IN FOLDER "\Schema Objects\Attributes" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->get_attribute_parent_candidates(	ATTRIBUTE => "attribute_name", 
					LOCATION => "location_path", 
					PROJECT => "project_name"
				     );
...
GET PARENT CANDIDATES FOR ATTRIBUTE "Month" IN FOLDER "\Schema Objects\Attributes" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->get_object_property(	PROPERTIES => [ "property1", "propertyN"], 
				PROJECT_CONFIG_OBJECT => (FOLDER | CONSOLIDATION | DOCUMENT | FILTER | METRIC | PROMPT | REPORT | SEARCH | TEMPLATE | ATTRIBUTE | FACT | FUNCTION | HIERARCHY | TABLE | TRANSFORMATION | DRILLMAP | SECFILTER | AUTOSTYLE | BASEFORMULA | SHORTCUT), 
				OBJECT_NAME => "object_name", 
				LOCATION => "location_path", 
				PROJECT => "project_name"
			);
...
GET PROPERTIES ID FROM ATTRIBUTE "Customer" IN FOLDER "\Schema Objects\Attributes\Customers" FOR PROJECT "MicroStrategy Tutorial";

GET PROPERTIES DESCRIPTION FROM ATTRIBUTE "Customer" IN FOLDER "\Schema Objects\Attributes\Customers" FOR PROJECT "MicroStrategy Tutorial";

GET PROPERTIES LOCATION FROM ATTRIBUTE "Customer" IN FOLDER "\Schema Objects\Attributes\Customers" FOR PROJECT "MicroStrategy Tutorial";

GET PROPERTIES CREATIONTIME FROM ATTRIBUTE "Customer" IN FOLDER "\Schema Objects\Attributes\Customers" FOR PROJECT "MicroStrategy Tutorial";

GET PROPERTIES MODIFICATIONTIME FROM ATTRIBUTE "Customer" IN FOLDER "\Schema Objects\Attributes\Customers" FOR PROJECT "MicroStrategy Tutorial";

GET PROPERTIES OWNER FROM ATTRIBUTE "Customer" IN FOLDER "\Schema Objects\Attributes\Customers" FOR PROJECT "MicroStrategy Tutorial";

GET PROPERTIES LONGDESCRIPTION FROM ATTRIBUTE "Customer" IN FOLDER "\Schema Objects\Attributes\Customers" FOR PROJECT "MicroStrategy Tutorial";

GET PROPERTIES HIDDEN FROM ATTRIBUTE "Customer" IN FOLDER "\Schema Objects\Attributes\Customers" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->get_tables_from_expression(	EXPRESSION => "expression", 
					PROJECT => "project_name"
				);

GET TABLES FROM EXPRESSION "<expression>" IN PROJECT "<project_name>";

GET TABLES FROM EXPRESSION "([QTY_SOLD] * (([UNIT_PRICE] - DISCOUNT) - [UNIT_COST]))" IN PROJECT "MicroStrategy Tutorial";

GET TABLES FROM EXPRESSION "TOT_DOLLAR_SALES - TOT_COST" IN PROJECT "MicroStrategy Tutorial";
...
$foo->grant_privileges(	PRIVILEGE => ["privilege1", "privilegeN"], 
			USER => "login_name", 
			GROUP => "user_group_name", 
...
$foo->grant_privileges(	PRIVILEGE => [qw(CREATESCHEMAOBJECTS SCHEDULEREQUEST USEOBJECTMANAGER USEVLDBEDITOR)], 
			USER => "Developer" );
			
GRANT WEBDRILLING, WEBEXPORT, WEBOBJECTSEARCH, WEBSORT, WEBUSER, WEBADMIN TO GROUP "Managers";

$foo->grant_privileges(	PRIVILEGE => [qw(WEBDRILLING WEBEXPORT WEBOBJECTSEARCH WEBSORT WEBUSER WEBADMIN)], 
			GROUP => "Managers" );

GRANT USESERVERCACHE, USECUSTOMGROUPEDITOR, USEMETRICEDITOR TO SECURITY ROLE "Normal Users";

$foo->grant_privileges(	PRIVILEGE => [qw(USESERVERCACHE USECUSTOMGROUPEDITOR USEMETRICEDITOR)], 
...
$foo->grant_security_roles(	SECURITY_ROLE => "sec_role_name",
				USER => "login_name", 
				GROUP => "user_group_name", 
				PROJECT => "project_name"
			  );

Optional parameters: SECURITY => ROLE,USER => ,PROJECT => 
...
$foo->idle_project(	PROJECT => "project_name", 
...
IDLE PROJECT "MicroStrategy Tutorial" MODE FULL;
IDLE PROJECT "MicroStrategy Tutorial" MODE REQUEST;
...
$foo->invalidate_report_cache(INVALIDATE => ([ALL] REPORT CACHES [WHTABLE ""WH_Table_name""] | REPORT CACHE ""cache_name""), IN_PROJECT => "project_name")

Optional parameters: ALL => ,WHTABLE => "<WH_Table_name>"
...
@$self{keys %parms} = values %parms;
my $result;
my @order = qw(INVALIDATE PROJECT);
my @required = qw();
for(@required){
	if(!defined($self->{$_})) { croak("\nRequired parameter not defined: " , $_, "\n"); }
}
...
for(@selected) {
	/INVALIDATE/ && do { $result .= "INVALIDATE " . $self->{INVALIDATE} . " "};
	/PROJECT/ && do { $result .= "IN PROJECT " . $self->{PROJECT} . " "};
}
...
$foo->list_acl_properties( 	OBJECT_TYPE => "configuration_object_type" | "project_object_type" | "FOLDER",
				OBJECT_NAME => "object_name",
				LOCATION => "location_path",
				PROJECT => "project_name"
			);
...
LIST ALL PROPERTIES FOR ACL FROM USER "Developer";

LIST ALL PROPERTIES FOR ACL FROM SCHEDULE "All the time";
...
$foo->list_attribute_properties(	ATTRIBUTE => "attribute_name", 
					LOCATION => "location_path", 
					PROJECT => "project_name"
				);
...
LIST ALL PROPERTIES FOR ATTRIBUTE "Customer" IN FOLDER "\Schema Objects\Attributes\Customers" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->list_attributes(	LOCATION => "location_path", 
			PROJECT => "project_name"
		     );
...
$foo->list_caching_properties(	CACHE_TYPE => (REPORT | OBJECT | ELEMENT), 
				PROJECT => "project_name"
			     );
...
$foo->list_custom_group_properties(	CUSTOMGROUP => "customgroup_name", 
					LOCATION => "location_path", 
					PROJECT => "project_name"
				  );
...
LIST ALL PROPERTIES FOR CUSTOMGROUP "Age Groups" IN FOLDER "\Public Objects\Custom Groups" FOR PROJECT "MicroStrategy Tutorial";
...
LIST ALL CUSTOMGROUPS FOR PROJECT "MicroStrategy Tutorial";
LIST ALL CUSTOMGROUPS IN FOLDER "\Public Objects\Custom Groups\" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->list_custom_groups(	LOCATION => "location_path", 
				PROJECT => "project_name"
			);
...
$foo->list_dbconnection_properties(	PROPERTIES => [ "property_name1", "property_nameN"], 
					DBCONNECTION => "dbConnection_name")
...
if(! $parms{PROPERTIES}) { $result .= "LIST ALL PROPERTIES "; };
for(@selected) {
...
$foo->list_dblogin_properties(	PROPERTIES => ["property_name1", "property_nameN"], 
				DBLOGIN => "dblogin_name"
			     );

Optional parameters: PROPERTIES=> <property_name1> , ... <property_nameN>
...
LIST PROPERTIES FOR DATABASE CONNECTION 1;
...
$foo->list_fact_properties(	FACT => "fact_name", 
				LOCATION => "location_path", 
				PROJECT => "project_name"
			  );
...
LIST ALL PROPERTIES FOR FACT "Profit" IN FOLDER "\Public Objects" FOR PROJECT "MicroStrategy Tutorial";
LIST ALL PROPERTIES FOR FACT "Revenue" IN FOLDER "\Public Objects" FOR PROJECT "MicroStrategy Tutorial";
LIST ALL FACTS IN FOLDER "\Schema Objects\Facts" FOR PROJECT "MicroStrategy Tutorial";
LIST ALL FACTS FOR PROJECT "MicroStrategy Tutorial";
...
$foo->list_facts(	LOCATION => "location_path", 
			PROJECT => "project_name"
		);
...
$foo->list_filter_properties(	FILTER => "filter_name", 
				LOCATION => "location_path", 
				PROJECT => "project_name"
			   );
...
LIST ALL PROPERTIES FOR FILTER "East Region" IN FOLDER "\Public Objects\Filters" FROM PROJECT "MicroStrategy Tutorial";
LIST ALL FILTERS IN "\Public Objects\Filters" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->list_filters(	LOCATION => "location_path", 
			OWNER => "login_name", 
			PROJECT => "project_name"
		  );
...
$foo->list_folder_properties(	FOLDER => "folder_name", 
				LOCATION => "location_path", 
				PROJECT => "project_name"
			    );
...
$foo->list_folders(	LOCATION => "location_path", 
			PROJECT => "project_name"
		  );
...
LIST ALL PROPERTIES FOR JOB 123;
...
$foo->list_metric_properties(	METRIC => "metric_name", 
				LOCATION => "location_path", 
				PROJECT => "project_name"
			    );
...
LIST ALL PROPERTIES FOR METRIC "New Metric" IN FOLDER "\Public Objects\Metrics" FROM PROJECT "MicroStrategy Tutorial";
...
$foo->list_metrics(	LOCATION => "location_path", 
			OWNER => "login_name", 
			PROJECT => "project_name"
		  );
...
LIST ALL METRICS FOR PROJECT "MicroStrategy Tutorial";
LIST ALL METRICS IN FOLDER "\Public Objects\Metrics\Count Metrics" FOR PROJECT "MicroStrategy Tutorial";
...
LIST ALL PROPERTIES FOR PROJECT CONFIGURATION FROM PROJECT "MicroStrategy Tutorial";
...
$foo->list_project_properties(	PROPERTIES => [ "prop_name1" "prop_nameN"], 
				PROJECT => "project_name"
			     );

Optional parameters: ALL => ,, => <prop_name2> , ... <prop_nameN>
...
LIST ID, NAME, CREATIONTIME, MODIFICATIONTIME FOR PROJECT "MicroStrategy Tutorial";
LIST ALL PROPERTIES FOR PROJECT "MicroStrategy Tutorial";
...
$foo->list_report_cache_properties(	REPORT_CACHE => "cache_name", 
					PROJECT => "project_name"
				  );
...
$foo->list_report_properties(	REPORT => "report_name", 
				LOCATION => "location_path", 
				PROJECT => "project_name"
			    );
...
$foo->list_reports(	LOCATION => "location_path", 
			PROJECT => "project_name"
		   );
...
$foo->list_schedule_relations(	SCHEDULE => "schedule_name", 
				USER_OR_GROUP => (USER | GROUP), 
				USER_LOGIN_OR_GROUP_NAME => "user_login_or_group_name", 
				REPORT => "rep_or_doc_name", 
				LOCATION => "location_path", 
...
LIST ALL SCHEDULERELATIONS FOR USER "crosie" IN PROJECT "MicroStrategy Tutorial";
LIST ALL SCHEDULERELATIONS FOR GROUP "Everyone" IN PROJECT "MicroStrategy Tutorial";
LIST ALL SCHEDULERELATIONS FOR REPORT "Revenue vs. Forecast" IN "\Public Objects\Reports\Subject Areas\Sales and Profitability Analysis"  IN PROJECT "MicroStrategy Tutorial";
...
$foo->list_security_filter_properties( 	PROPERTIES => [ "property_name1", "property_nameN"], 
...
LIST ALL PROPERTIES FOR SECURITY FILTER "SecFilter1" OF PROJECT "MicroStrategy Tutorial";
LIST ALL PROPERTIES FOR SECURITY FILTER "SecFilter3" FOLDER  "\Project Objects\MD Security Filters" OF PROJECT "MicroStrategy Tutorial";
...
$foo->list_security_filters(	USER_OR_GROUP => "USER" | "GROUP, 
				USER_LOGIN_OR_GROUP_NAME => "user_login_or_group_name", 
				LOCATION => "location_path", 
				PROJECT => "project_name"
			   );
...
$foo->list_security_role_properties(	PROPERTIES => ["property_name1", "property_nameN"], 
					SECURITY_ROLE => "sec_role_name"
				     );
...
LIST NAME, ID FOR SECURITY ROLE "Normal Users";
...
LIST PRIVILEGES FOR SECURITY ROLE "Architect Users";
...
$foo->list_server_config_properties(ALL | [property_name1, property_nameN]);
...
LIST PROPERTIES FOR SERVER "p_alcazar";
...
$foo->list_shortcut_properties(	LOCATION => "location_path", 
...
LIST ALL PROPERTIES FOR SHORTCUT IN FOLDER "\Public Objects" FOR FILTER "Age Groups" FOR PROJECT "MicroStrategy Tutorial";
LIST ALL PROPERTIES FOR SHORTCUT IN FOLDER "\Public Objects" FOR HIERARCHY "Time (Browsing)" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->list_shortcuts(	LOCATION => "location_path", 
			PROJECT => "project_name"
		    );
...
LIST ALL SHORTCUTS FOR PROJECT "Microstrategy Tutorial";
...
$foo->list_table_properties(	TABLE => "table_name", 
				PROJECT => "project_name"
			   );
...
LIST ALL PROPERTIES FOR TABLE "ORDER_FACT" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->list_tables(	LOCATION => "location_path", 
			PROJECT => "project_name"
		 );
...
LIST ALL TABLES IN FOLDER "\Schema Objects\Tables" FOR PROJECT "MicroStrategy Tutorial";
...
LIST ALL PROPERTIES FOR USER CONNECTION SESSIONID 849C4BBC11D353F2C000159051708F4F;
...
LIST PRIVILEGES FOR USER GROUP "Managers";
...
$foo->list_user_group_properties(	PROPERTIES => ["property_name1", "property_nameN"], 
...
LIST NAME, DESCRIPTION, MEMBERS FOR USER GROUP "Managers";
...
if(!defined($self->{PROPERTIES})) {$result .= "LIST ALL PROPERTIES "; }
for(@selected) {
	/PROPERTIES/ && do { $result .= "LIST " . join(", ", @{$self->{PROPERTIES}}) . " "};
...
$foo->list_user_privileges(	TYPE => (ALL | INHERITED | GRANTED), 
				USER => "login_name")
...
LIST ALL PRIVILEGES FOR USER "smiller";
LIST INHERITED PRIVILEGES FOR USER "ryoung";
LIST GRANTED PRIVILEGES FOR USER "oneal";
...
$foo->list_user_profiles( 	USER => "login_name" | "ALL",
				GROUP => "group_name" | [ "group_name1", "group_nameN"],
				PROJECT => "project_name" | [project_name1", "project_nameN]
			);

Optional parameters: GROUP, PROJECT
...
LIST PROFILES FOR USER "Developer" FOR PROJECT "MicroStrategy tutorial";
LIST PROFILES FOR USER "Developer" FOR PROJECTS "MicroStrategy Tutorial", "Customer Analysis Module";
LIST PROFILES FOR USER "Developer";
LIST PROFILES FOR USERS;
LIST PROFILES FOR USERS FOR PROJECTS "MicroStrategy Tutorial", "Customer Analysis Module";
LIST PROFILES FOR USERS IN GROUP "Customers";
LIST PROFILES FOR USERS IN GROUPS "Customers", "Developers" FOR PROJECTS "MicroStrategy Tutorial", "Customer Analysis Module";
...
$foo->list_user_properties(	PROPERTIES => [property_name1, property_nameN ], 
				USER => "login_name",
				USER_GROUP => "user_group_name"
			  );
...
LIST NAME, FULLNAME, DESCRIPTION, NTLINK, PASSWORDEXP FOR USER "palcazar";
LIST ALL PROPERTIES FOR USERS IN GROUP "MicroStrategy Tutorial Managers";
...
LIST ALL AVAILABLE WHTABLES FOR PROJECT "MicroStrategy Tutorial";
...
$foo->load_projects_cluster(	PROJECT => "project_name", 
...
LOAD PROJECT "MicroStrategy Tutorial" TO CLUSTER SERVERS "WAS-PLE", "WAS_AJHAJHARIA";

LOAD PROJECT "MicroStrategy Tutorial" TO CLUSTER ALL SERVERS;
...
$foo->lock_project(	PROJECT => "project_name",
			FORCE => "TRUE" | "FALSE"
		  );
...
$foo->log_event(	MESSAGE => "event_message", 
...
$foo->privileges_list("privilege_group" | [ "privilege_group1", "privilege_groupN" ] );
...
$foo->purge_caching(	TYPE => (ALL | OBJECT | ELEMENT | REPORT), 
...
$foo->purge_statistics( 	START_DATE => "start_date", 
				END_DATE => "end_date", 
				TIMEOUT => "seconds"
		      );
...
$foo->register_project(	PROJECT => "project_name"
		      	AUTOLOAD => "TRUE" | "FALSE"
		      );
...
REGISTER PROJECT "MicroStrategy Tutorial";
...
$foo->remove_attribute_child(	ATTRIBUTECHILD => "attributechild_name", 
				ATTRIBUTE => "attribute_name", 
				LOCATION => "location_path", 
				PROJECT => "project_name"
			    );
...
REMOVE ATTRIBUTECHILD "Day" FROM ATTRIBUTE "Month" IN FOLDER "\Schema Objects\Attributes" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->remove_attribute_form_expression(	ATTRIBUTEFORMEXP => "expression", 
					ATTRIBUTEFORM => "form_name", 
					ATTRIBUTE => "attribute_name", 
					LOCATION => "location_path", 
					PROJECT => "project_name"
...
REMOVE ATTRIBUTEFORMEXP "ORDER_DATE" FROM ATTRIBUTEFORM "ID" FOR ATTRIBUTE "Day" IN FOLDER "\Schema Objects\Attributes" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->remove_attribute_form(	ATTRIBUTEFORM => "form_name", 
				ATTRIBUTE => "attribute_name", 
				LOCATION => "location_path", 
				PROJECT => "project_name"
			   );
...
REMOVE ATTRIBUTEFORM "Last Name" FROM ATTRIBUTE "Customer" IN FOLDER "\Schema Objects\Attributes" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->remove_attribute_parent(	ATTRIBUTEPARENT => "attributeparent_name", 
				ATTRIBUTE => "attribute_name", 
				LOCATION => "location_path", 
				PROJECT => "project_name"
			     );
...
REMOVE ATTRIBUTEPARENT "Month of Year" FROM ATTRIBUTE "Month" IN FOLDER "\Schema Objects\Attributes" FOR PROJECT "MicroStrategy Tutorial";
REMOVE ATTRIBUTEPARENT "Quarter" FROM ATTRIBUTE "Month" IN FOLDER "\Schema Objects\Attributes" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->remove_configuration_ace(	CONF_OBJECT_TYPE => "conf_object_type", 
				OBJECT_NAME => "object_name", 
				USER_OR_GROUP => "USER" | "GROUP", 
				USER_LOGIN_OR_GROUP_NAME => "user_login_or_group_name"
			      );
...
REMOVE ACE FROM SCHEDULE "All the Time" USER "Developer";
...
$foo->remove_custom_group_element(	ELEMENT => "element_name", 
					CUSTOMGROUP => "customgroup_name", 
					LOCATION => "location_path", 
					PROJECT => "project_name"
				 );
...
REMOVE ELEMENT "25-35" FROM CUSTOMGROUP "Copy of Age Groups" IN FOLDER "\Public Objects\Custom Groups" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->remove_dbinstance(	DBINSTANCE => "DBInstance_name", 
				PROJECT => "project_name"
			);
...
$foo->remove_fact_expression(	EXPRESSION => "expression", 
				FACT => "fact_name", 
				LOCATION => "location_path", 
				PROJECT => "project_name"
			    );
...
REMOVE EXPRESSION "ORDER_ID" FROM FACT "Profit" IN FOLDER "\Public Objects" FOR PROJECT "MicroStrategy Tutorial";
REMOVE EXPRESSION "ORDER_AMT - ORDER_COST" FROM FACT "Profit" IN FOLDER "\Public Objects" FOR PROJECT "MicroStrategy Tutorial";
REMOVE EXPRESSION "([QTY_SOLD] * (([UNIT_PRICE] - DISCOUNT) - [UNIT_COST]))" FROM FACT "Profit" IN FOLDER "\Public Objects" FOR PROJECT "MicroStrategy Tutorial";

REMOVE EXPRESSION "ITEM_ID" FROM FACT "Cost" IN FOLDER "\Public Objects" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->remove_folder_ace(	FOLDER => "folder_name", 
				LOCATION => "location_path", 
				USER_OR_GROUP => "USER" | "GROUP", 
				USER_LOGIN_OR_GROUP_NAME => "user_login_or_group_name", 
				PROJECT => "project_name"
...
REMOVE ACE FROM SCHEDULE "All the Time" USER "Developer";
REMOVE ACE FROM FOLDER "Subtotals" IN FOLDER "\Project Objects" USER "Developer" FOR PROJECT "MicroStrategy Tutorial";
...
$foo->remove_project_ace(	PROJECT_OBJECT_TYPE => "project_object_type", 
				OBJECT_NAME => "object_name", 
				LOCATION => "location_path", 
				USER_OR_GROUP => "USER" | "GROUP", 
				USER_LOGIN_OR_GROUP_NAME => "user_login_or_group_name",
...
REMOVE SERVER "PROD_SRV2" FROM CLUSTER;
...
$foo->remove_user(	USER => "login_name", 
			GROUP => "group_name" | ["group_name1", "group_nameN"]
		 );
...
REMOVE USER "palcazar" FROM GROUP "Public / Guest";
...
$foo->remove_whtable(	WHTABLE => "warehouse_table_name" 
			TABLE => "table_name", 
			PROJECT => "project_name"
		    );
...
foo->revoke_privileges(	PRIVILEGE => "ALL" | ["privilege1", "privilegeN"], 
			USER => "login_name", 
			GROUP => "user_group_name", 
			SECURITY_ROLE => "sec_role_name"
		      );
...
$foo->revoke_security_filter(	SECURITY_FILTER => "sec_filter_name", 
				LOCATION => "location_path", 
				USER_OR_GROUP => "USER" | "GROUP", 
...
REVOKE SECURITY FILTER "SecFilter1" FOLDER "\Project Objects\MD Security Filters" FROM USER "Developer" ON PROJECT "MicroStrategy Tutorial";
...
$foo->revoke_security_roles(	SECURITY_ROLE => "sec_role_name",
				USER_OR_GROUP => "USER" | "GROUP", 
...
REVOKE SECURITY ROLE "Normal Users" FROM USER "Developer" ON PROJECT "MicroStrategy Tutorial";
REVOKE SECURITY ROLE "Power Users" FROM GROUP "Managers" ON "MicroStrategy Tutorial";
...
$foo->send_message( 	MESSAGE => "message", 
			USER => "login_name" | "ALL" 
		  );
...
SEND MESSAGE "You are about to be disconnected. Please save your reports." TO USER "Developer";
SEND MESSAGE "The IServer will be shutdown. Please save your work." TO ALL USERS;
...
$foo->set_property_hidden(	HIDDEN => "TRUE" | "FALSE", 
...
$foo->start_service(	SERVICE => "service_name", 
			SERVER => "machine_name"
		   );
...
$foo->stop_service(	SERVICE => "service_name", 
			SERVER => "machine_name"
		  );
...
$foo->take_ownership(	OBJECT_TYPE => "conf_object_type" | "project_object_type" | "FOLDER",
			OBJECT_NAME => "object_name",			
...
$foo->unload_projects_cluster(	PROJECT => "project_name", 
				SERVERS =>  "ALL" | ["server_name1", "server_nameN" ]
			    );
...
UNLOAD PROJECT "MicroStrategy Tutorial" FROM CLUSTER SERVERS "WAS-PLE", "WAS_AJHAJHARIA";
UNLOAD PROJECT "MicroStrategy Tutorial" FROM CLUSTER ALL SERVERS;
...
UNREGISTER PROJECT "MicroStrategy Tutorial";
...
$foo->update_schema(	REFRESHSCHEMA => "TRUE" | "FALSE",
...
Optional parameters:  	REFRESHSCHEMA => "TRUE" | "FALSE",
			RECALTABLEKEYS	 => "TRUE" | "FALSE",
			RECALTABLELOGICAL => "TRUE" | "FALSE",			
			RECALOBJECTCACHE => "TRUE" | "FALSE",
...
$foo->update_structure(	COLMERGEOPTION => (RECENT | MAXDENOMINATOR | NOMERGE), 
			WHTABLE => "warehouse_table_name", 
			PROJECT => "project_name"
		      );

Optional parameters: FOR => WHTABLE "<warehouse_table_name>"
...
Please report any bugs or feature requests to C<bug-microstrategy-cmdmgr at rt.cpan.org>, or through
the web interface at L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=MicroStrategy-Cmdmgr>.  I will be notified, and then you'll
...
L<http://rt.cpan.org/NoAuth/Bugs.html?Dist=MicroStrategy-Cmdmgr>

=item * AnnoCPAN: Annotated CPAN documentation

L<http://annocpan.org/dist/MicroStrategy-Cmdmgr>

=item * CPAN Ratings

L<http://cpanratings.perl.org/d/MicroStrategy-Cmdmgr>

=item * Search CPAN

L<http://search.cpan.org/dist/MicroStrategy-Cmdmgr>
